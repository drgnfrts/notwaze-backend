...
def generate_full_route(user_data: UserData, route_points_gdf: GeoDataFrame, nearby_poi_gdf: GeoDataFrame, nearby_amenities_gdf: GeoDataFrame, avoidance_buffer_gdf: GeoDataFrame):
    ...
    while i < len(route_points_gdf) - 1:
        ...
        print(f"Attempting to find route between {current_point_gdf['NAME']} and {next_point_gdf['NAME']}")
        ...
        if latest_distance >= CUT_OFF and (next_point != end_point):
            inserted_amenity, amenity_distance = find_nearest_amenity(current_point, nearby_amenities_gdf)
            ...
            if distance_to_amenity + distance_from_amenity + metadata['total_distance'] < max_route_length:
                metadata = update_metadata(route_geometry_to_amenity, time_to_amenity, distance_to_amenity)
                metadata = update_metadata(route_geometry_after_amenity, time_after_amenity, distance_from_amenity)
        ...
        # update metrics
        metadata = update_metadata(metadata, route_geometry, latest_time, latest_distance)
        avoidance_check_attempts = 0
        i += 1
...
def handle_backtrack(metadata: dict, end_point_gdf: GeoDataFrame, max_route_length: int):
    ...
    while metadata["total_distance"] >= max_route_length and len(metadata['final_points_gdf_list']) > 1:
        ...
        metadata["total_time"] -= metadata["route_time"].pop()
...
def update_metadata(metadata: dict, point_gdf: GeoDataFrame, route_geometry: LineString, latest_time, latest_distance):
    ...
    metadata["route_time"].append(latest_time)
...
def find_nearest_amenity(point_gdf, amenity_gdf):
  ...
  new_point_gdf = point_gdf.copy().to_crs(epsg=3414)

  amenity_gdf.to_crs(epsg=3414, inplace=True)

  # Find distance between point and amenity in amenity_gdf
  amenity_gdf['distance'] = amenity_gdf.geometry.apply(lambda x: new_point_gdf.geometry.distance(x).min())
